...
/*==============================================================================
 * 1. 轨迹编号 / Trajectory ID
 *============================================================================*/
/**
 * @brief BDPS 优化轨迹编号。
 *
 * 输入：由 BDPS_CalcD1_Optimized() 根据 K 和 p0 自动选择。
 * 输出：写入 BDPS_OptResult_t::traj_id，方便调试观察。
 */
typedef enum {
    BDPS_TRAJ_NONE = 0,
    BDPS_TRAJ_R1,
 ...
 *============================================================================*/
/**
 * @brief BDPS 算法输入、中间量、输出结果。
 *
 * 输入字段：
 *   U1 / U2 / I2 / D2_in 保存本次算法入口参数。
 *
 * 输出字段：
 *   K / PN / Pout / p0 / A / B / C 保存中间计算量。
 *   D2 / D1 / traj_id / err_flags 保存最终计算结果和状态。
 */
typedef struct {
    float32_t U1;                /* 原边电压采样 */
    float32_t U2;                /* 副边电压采样 */
